...
//        data.put(data.size(), "rtsp://218.204.223.237:554/live/1/66251FC11353191F/e7ooqwcfbqjoo80j.sdp")
        data.put(data.size(), "")
        data.put(data.size(), "http://cdn.phpec.org/video/pano.mp4")
...
//        data.put(data.size(), sPath + "WSGV6301.jpg")
        ...
        findViewById(R.id.ijk_button).setVisibility(View.GONE)
        findViewById(R.id.bitmap_button).setVisibility(View.GONE)

        findViewById(R.id.normal_button).setOnClickListener(View.OnClickListener {
            val url = et.text.toString()
            if (!TextUtils.isEmpty(url)) {
                MD360PlayerActivity.startNormalVideo(this@DemoActivity, Uri.parse(url))
            } else {
        ...
        findViewById(R.id.about_button).setOnClickListener(View.OnClickListener {
                Toast.makeText(this@DemoActivity, "风子 http://www.phpec.org", Toast.LENGTH_LONG).show()
        })
    }
